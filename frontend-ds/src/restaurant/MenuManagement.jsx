...
const MenuManagement = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      const res = await fetch("http://localhost:5000/api/restaurant/profile/me", { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/menu", { method: "POST", headers: { Authorization: `Bearer ${token}` }, body: form });
      const data = await res.json();
      if (res.ok) {
  ...
  const handleDelete = async (id) => {
    ...
    const token = localStorage.getItem("restaurantToken");
    try {
      const res = await fetch(`http://localhost:5000/api/menu/${id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
      if (res.ok) setMenuItems(menuItems.filter((item) => item._id !== id));
  ...
  const toggleAvailability = async (item) => {
    ...
    try {
      ...
      if (res.ok) {
        setMenuItems((prev) => prev.map((i) => (i._id === item._id ? updated.updatedItem : i)));
      }
    } catch (err) {
        ...
        <Dialog open={isModalOpen} onClose={() => setIsModalOpen(false)} className="fixed z-10 inset-0 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen p-4">
            <Dialog.Panel className="w-full max-w-md bg-white p-6 rounded-lg shadow-xl">
              <Dialog.Title className="text-xl font-semibold mb-4">➕ Add Menu Item</Dialog.Title>
